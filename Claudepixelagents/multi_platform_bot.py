"""
Multi-Platform Bot Architect - Создание ботов для Telegram и Instagram
Groq и системные ключи в коде, остальные ключи в Telegram
"""
...
class MultiPlatformBot:
    def __init__(self, telegram_token: str, groq_api_key: str):
        logger.info("Инициализация мульти-платформенного бота...")
        ...
        self.platform_templates = {
            "telegram": {
                ...
                "example_usage": "/start, /menu, /booking, кнопки 'Заказать', 'Позвонить'"
            },
            "instagram": {
                "name": "Instagram Bot",
                ...
                "example_usage": "Авто-ответ на 'меню', 'цены', 'бронь', реакция на сторис"
            },
            "both": {
                ...
                "example_usage": "Один бот работает в Telegram и Instagram одновременно"
            }
        }
        
        logger.info(f"Загружено {len(self.platform_templates)} платформенных шаблонов")
    
    async def start(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        user_id = update.effective_user.id
        username = update.effective_user.username or "Unknown"
        
        logger.info(f"Пользователь {username} (ID: {user_id}) запустил мульти-платформенного бота")
        
        text = """🚀 *Multi-Platform Bot Architect готов!*
...
🧠 *Выбери платформу для создания бота:*"""
        ...
        try:
            ...
            logger.info(f"Меню платформ отправлено пользователю {user_id}")
...
📊 *Цена настройки:*
{self.get_platform_price(platform)}
...
🧠 *Далее опиши какой бот нужен*"""
        ...
        try:
            ...
            logger.info(f"Пользователь {user_id} выбрал платформу: {platform}")
            
        except Exception as e:
            logger.error(f"Ошибка при выборе платформы: {e}")
            await query.edit_message_text("❌ Произошла ошибка. Попробуйте еще раз.")
    
    def get_platform_price(self, platform: str) -> str:
        """Получить цену для платформы"""
        prices = {
            "telegram": "$100-200",
            "instagram": "$150-300",
            "both": "$300-500"
        }
        return prices.get(platform, "$100-200")
...
🔙 *Вернуться к выбору платформы*"""
    ...
    async def start_prompt_creation(self, update: Update):
        """Начать создание бота по промпту"""
        query = update.callback_query
        user_id = update.effective_user.id
...
✍️ *Введи свой промпт:*"""
        ...
        try:
            ...
            logger.info(f"Пользователь {user_id} начал создание бота для платформы {platform}")
            
        except Exception as e:
            logger.error(f"Ошибка при начале создания: {e}")
            await query.edit_message_text("❌ Произошла ошибка. Попробуйте еще раз.")
    ...
    async def process_prompt(self, update: Update, prompt: str):
        ...
        logger.info(f"Обрабатываю промпт для {platform} от пользователя {user_id}: {prompt[:100]}...")
...
💡 *Минимум:* {', '.join(template['api_requirements'][:2])}"""
        ...
        try:
            ...
            logger.info(f"Запросил API ключи для {platform} у пользователя {user_id}")
    ...
    async def process_api_keys(self, update: Update, keys_text: str):
        ...
        logger.info(f"Получил API ключи для {platform} от пользователя {user_id}")
        ...
        try:
            ...
            # Проверка обязательных ключей для платформы
            missing_keys = []
...
🔨 *Создаю {template['name']}...*"""
            ...
            logger.info(f"{template['name']} для пользователя {user_id} успешно создан")
    ...
    async def send_platform_bot(self, update: Update, config: dict):
        """Отправка бота для конкретной платформы"""
        try:
...
💰 *Монетизация:*
• Базовая настройка: {self.get_platform_price(platform)}
• Дополнительные модули: +$50-100
• Поддержка: $50/мес
• % с продаж: 5-15%

🔥 *{template['name']} готов к работе!*"""
            ...
            logger.info(f"{template['name']} отправлен пользователю")
            
        except Exception as e:
            logger.error(f"Ошибка при отправке бота: {e}")
...
def main():
    try:
        ...
        logger.info("🚀 Запуск Multi-Platform Bot Architect...")
        
        application = Application.builder().token(telegram_token).build()
        bot = MultiPlatformBot(telegram_token, groq_api_key)
        ...
        logger.info("✅ Multi-Platform Bot готов!")
        print("✅ Multi-Platform Bot Architect готов!")
        print("🎯 Поддерживаемые платформы: Telegram, Instagram, Multi-Platform")
